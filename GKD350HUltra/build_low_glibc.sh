...
CXX_CMD="${CROSS_CXX:-${TOOL_PREFIX}-g++-11}"
...
GCC_LIBDIR="$SYSROOT/usr/lib/gcc/aarch64-linux-gnu/11"
GCC_LIBDIR_ALT="$SYSROOT/lib/gcc/aarch64-linux-gnu/11"
...
export LIBRARY_PATH="$GCC_LIBDIR:$GCC_LIBDIR_ALT:$SYSROOT/usr/lib/aarch64-linux-gnu:$SYSROOT/lib/aarch64-linux-gnu${LIBRARY_PATH:+:$LIBRARY_PATH}"
...
for dep_file in "$OBJ_DIR"/*.d; do
  [ -f "$dep_file" ] || continue
  # Dependency files generated by MSYS2 contain drive-letter targets such as
  # `D:/...`; GNU make treats the colon as a second rule separator in WSL.
  # Drop those stale files and let the current toolchain regenerate them.
...
done
{
  ...
  nice -n 15 ionice -c 2 -n 7 make -j"$BUILD_JOBS" \
    ...
    EXTRA_LDFLAGS="--sysroot=$SYSROOT -L$GCC_LIBDIR -L$GCC_LIBDIR_ALT -L$LIBDIR -Wl,-rpath,'\$\$ORIGIN/lib' -Wl,-rpath,'\$\$ORIGIN/lib_system_sdl'" \
    all
} 2>&1 | tee "$LOG_FILE"
