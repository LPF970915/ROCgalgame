param(
  ...
  [string]$CorePath = "$PSScriptRoot\dist_lowglibc\ROCgalgame\cores\krkr\krkr2"
)
...
backup_dir="/storage/games-external/ROCgalgame_refactor_backups/krkr2_layerex_`$stamp"
mkdir -p "`$backup_dir"
if [ -f "`$target" ]; then cp -p "`$target" "`$backup_dir/krkr2"; fi
cp "`$incoming" "`$app/cores/krkr/.krkr2.aac.new"
chmod 755 "`$app/cores/krkr/.krkr2.aac.new"
sync
mv "`$app/cores/krkr/.krkr2.aac.new" "`$target"
sync
