macro(cocos2dx_depend)
    ...
    elseif(LINUX)
        if(ROCGALGAME_LINUX_GLES2)
            list(APPEND PLATFORM_SPECIFIC_LIBS dl rt m)
        else()
...
macro(use_cocos2dx_libs_depend target)
    ...
    if(LINUX)
        ...
        if(CMAKE_SYSROOT)
            foreach(variable GTK3_INCLUDE_DIRS GTK3_LIBRARY_DIRS)
                set(fixed_paths)
                foreach(path IN LISTS ${variable})
                    string(FIND "${path}" "${CMAKE_SYSROOT}/mnt/" duplicate_sysroot)
                    string(FIND "${path}" "${CMAKE_SYSROOT}/workspace/" duplicate_workspace_sysroot)
                    string(FIND "${path}" "${CMAKE_SYSROOT}${CMAKE_SYSROOT}/" duplicate_self_sysroot)
                    if(duplicate_sysroot EQUAL 0 OR duplicate_workspace_sysroot EQUAL 0 OR duplicate_self_sysroot EQUAL 0)
                        string(LENGTH "${CMAKE_SYSROOT}" sysroot_length)
                        string(SUBSTRING "${path}" ${sysroot_length} -1 path)
                    endif()
                    list(APPEND fixed_paths "${path}")
                endforeach()
                set(${variable} "${fixed_paths}")
            endforeach()
        endif()
        ...
        if(ROCGALGAME_LINUX_GLES2)
            target_compile_definitions(${target} PUBLIC ROCGALGAME_LINUX_GLES2=1)
            target_link_libraries(${target} PUBLIC EGL GLESv2)
        else()
