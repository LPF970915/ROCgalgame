...
if (-not $Force -and ((Test-Path -LiteralPath $packagePath) -or (Test-Path -LiteralPath $checksumPath))) {
  throw "Release output already exists; use -Force to replace version $Version"
}
...
wsl -d $Distro -- docker run --rm `
  ...
  --env "ROC_RELEASE_FORCE=$([int]$Force.IsPresent)" `
