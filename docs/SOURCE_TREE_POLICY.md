...
This project uses three separate working trees. They have different owners and
...
2. KRKR2 changes are committed in `ROCgalgame-krkr2-port`. KRKRSDL2 changes are
   committed in `ROCgalgame-krkrsdl2-port`. The main repository stores build
...
4. CMake build directories are caches, never source-of-truth. The expected
   container paths are `/sources/krkr2`, `/sources/krkrsdl2`, and `/workspace`.
...
2. Verify the three source trees and lock commits.
